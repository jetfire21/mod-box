	...
	<div class="container">
			...
			<div class="cl-menu">
				...
				<!-- выводит все статьи категории -->
				 <?php
					...
					$query = new WP_Query( array( 'category_name' => $slug, 'orderby' => 'date', 'order' => 'ASC') );
					// print_r($query);
				  ?>
				 <?php if( !is_category() ) $cur_post_id = get_the_ID(); else $cur_post_id = $query->posts[0]->ID;?>
			    <?php if($query->have_posts() ): ?>
				    <ul>
				    <?php while($query->have_posts() ) : $query->the_post();?>    
							<li>
								<a class="cl-menu-item <?php if( $cur_post_id == $post->ID ) echo 'cl-active';?>" data-post-id="<?php echo $post->ID;?>" href="<?php echo get_permalink();?>">
								<?php
								...
								<p class="titile-for-send-mail">
								<?php echo the_title(); ?>
								</p>

								<?php $main_price = get_post_meta($post->ID, 'main_price', true); ?>
								<?php if($main_price):?>
								<span> <p class="price-for-send-mail"><?php echo $main_price;?></p> <span class="rub">в</span></span>
								<?php endif;?>
								</a>
							</li>
				    <?php endwhile; ?>
		...
		<div class="content-right-info">
			...
			<?php if( !is_category() ): ?>
				     ...
				     <?php if( have_posts() ):
							...
							<div class="cr-title">
								...
								<a class="cr-but mail-form2" href="#"><i class="cr-calc"></i>Оформить заказ</a>
								   <div class="clear"></div>
							</div>	
							 <div class="cr-text">	   
					           <?php the_content(); ?>
			...
			<?php else: ?>	
					  ...
					  <?php
					  //  сортировка по дате - последняя дата - самое начало
						$latest_cat_post = new WP_Query( array('posts_per_page' => 1, 'category__in' => array($cats->cat_ID), "orderby" => "date", "order" => "ASC") );
